...
const getData = async (url, cbSuccess, cbError) => {
  try {
    /* await =  подожди пока выполнится запрос*/
    const response = await fetch(url);
    const data = await response.json(); // из json в нормальный тип данных
    cbSuccess(data);
  } catch (err) {
    cbError(err);
  }
...
const createCard = (vacancy) => `
  <article class="vacancy" tabindex="0" data-id="${vacancy.id}">
    <img class="vacancy__img" src="${API_URL}${vacancy.logo}"
  ...
  <article class="detail">
            ...
            <div class="detail__main">
               ...
               <ul class="detail__fields">
                  <li class="detail__field">${parseInt(
                    salary
                  ).toLocaleString()}₽</li>
